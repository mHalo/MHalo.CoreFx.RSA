...
namespace MHalo.CoreFx.Helper
{
    ...
    public static class RSAHelper
    {
        ...
        /// <returns>加密后的字符</returns>
        /// <returns></returns>
        public static string EncryptByPrivateKey(RSAKeyType keyType, string content, string privateKeyContent, CipherAlgorithm cipherAlgorithm = CipherAlgorithm.RSA_ECB_PKCS1Padding)
        {
            AsymmetricKeyParameter privateKeyParameter = RSAKeyExtensions.CreateAsymmetricPrivateKeyParameter(keyType, privateKeyContent);
            IBufferedCipher cipher = CipherUtilities.GetCipher(cipherAlgorithm.GetAlgorithm());
        ...
        public static string SignData(RSAKeyType keyType, string data, string privateKeyContent, SignerAlgorithm signerAlgorithm = SignerAlgorithm.SHA256withRSA)
        {
            ...
            ArgumentNullException.ThrowIfNull(privateKeyContent);
        ...
        /// MD5withRSA
        /// </param>
        /// <returns>验证成功返回 true，验证失败返回 false，密钥或数据格式错误抛出异常</returns>
        public static bool VerifyData(RSAKeyType keyType, string data, string sign, string publicKeyContent, SignerAlgorithm signerAlgorithm = SignerAlgorithm.SHA256withRSA)
        {
            ArgumentNullException.ThrowIfNull(data);
            ArgumentNullException.ThrowIfNull(sign);
            ArgumentNullException.ThrowIfNull(publicKeyContent);
